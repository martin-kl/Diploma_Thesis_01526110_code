# Databases and how to start them + LDBC
## Neo4j
...
- start: run virtualBox image, ``gadmin start all`` in shell

<!-- 
enable Drag- Drop to Tigergraph image in VirtualBox:
add guest additions + install them
then run following commands
- sudo apt-get install virtualbox-guest-dkms 
- sudo apt-get install virtualbox-guest-utils
- sudo apt-get install virtualbox-ext-pack
restart, then enable Drag & Drop
-->
...
- run these commands in WSL in folder ``titan-1.0.0-hadoop1/bin``:
    - remove everything if needed: ``./titan.sh stop && ./titan.sh clean``
    - or just start titan with default cassandra storage backend (everything in WSL): ``./titan.sh start``
    - start gremlin: ``./gremlin.sh``

- [exemplary connection to a graph on cassandra](http://s3.thinkaurelius.com/docs/titan/1.0.0/getting-started.html#_loading_the_graph_of_the_gods_into_titan):
...
- download everything from git repo, open ``docker_run.sh`` and save it with LF instead of CR LF (Windows)  
...
- run following commands from .../ldbc_snb_datagen/ folder:
    ...
    ```
    - run docker in normal cmd/powershell via
    ```
...
## Neo4j
1. run `convert-csvs.sh` in bash (**Attention**: files are then *not* usable to e.g. be imported into TigerGraph after this command)
    - first:
    ```
    export NEO4J_DATA_DIR=/<path>/ldbc/ldbc_snb_datagen/social_network
...
3. import
    ...
    - ensure that following Environment variables are set:
        ``NEO4J_DATA_DIR=<path>\ldbc\ldbc_snb_datagen\social_network``  
        ``POSTFIX=_0_0.csv``  
        *Windows note: after changing one of these variables we have to restart terminal, e.g. ConEmu*
...
## TigerGraph
- download ecosys from GitHub  
    *(Note: this is too big to clone via git -> google for svn method to only download the ldbc_benchmark/tigergraph directory)*
- either generate data in virtual machine or simply copy files in there 
    - (enable Drag & Drop before; Note: drag & Drop files from each folder, subfolder does not work automatically)
...
- [note from earlier tryouts:] *ATTENTION*: simply running ``./one_step_load.sh`` did not work, I think cause it's too much data (times out) even on scale 0.1 -> import in parts (long runtime potentially caused by index creation):
...
### Install the Queries
- **Attention:** I took the queries from the ecosys git - but not from the main directory but from: ``ldbc_benchmark/tigergraph/queries_linear/queries`` (December 2020: note that this folder does not exist any more!)
- these queries are based on the schema: ``ldbc/ldbc_benchmark/tigergraph/gsql102/3.0/setup_schema.gsql``) 
...
- load data into Titan:
  ...
  ``-C 127.0.0.1 -input <path>\ldbc\ldbc_snb_datagen\social_network_titan -batchSize 512 -graphName default -progReportPeriod 10`` (Attention: runs quite long, >20 mins)
...
## Cypher
- copy queries to online Query editor (http://localhost:7474/browser/) and run it there
...
---
## GSQL
Tigergraph in VirtualBox, queries already installed  
