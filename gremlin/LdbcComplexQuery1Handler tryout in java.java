...
public class LdbcComplexQuery1Handler implements OperationHandler<LdbcQuery1, DbConnectionState> {
  ...
  public void executeOperation(LdbcQuery1 ldbcQuery1, DbConnectionState dbConnectionState, ResultReporter resultReporter) throws DbException {
    ...
    */
    ...
    Graph graph = ((GremlinDbConnectionState) dbConnectionState).getClient();
  }

  public static void main(String[] args) {
    matchList = [];
    distList = [];
    g.withSideEffect('x', matchList).withSideEffect('d', distList).V().has(person_label, 'iid', person_id).aggregate('done')
        .out('knows').where(without('done')).dedup().fold()
        .sideEffect(unfold().has('firstName', firstName).order().by('lastName', incr).by('iid_long', incr).limit(result_limit).as('person').select('x').by(count(Scope.local)).is(lt(result_limit)).store('x').by(select('person')))
        .filter(select('x').count(Scope.local).is(lt(result_limit)).store('d')).unfold().aggregate('done').out('knows').where(without('done')).dedup().fold()
        .sideEffect(unfold().has('firstName', firstName).order().by('lastName', incr).by('iid_long', incr).limit(result_limit).as('person').select('x').by(count(Scope.local)).is(lt(result_limit)).store('x').by(select('person')))
    ...
    Map<Vertex, Map<String, List<String>>> propertiesMap = new HashMap<>();
    g.V().has(person_label, 'iid', within(matchListIds)).as('person').<List<String>>valueMap().as('props').select('person', 'props').forEachRemaining({map -> propertiesMap.put((Vertex) map.get('person'), (Map<String, List<String>>) map.get('props'))});

    placeNameMap = [:];
    g.V().has(person_label, 'iid', within(matchListIds)).as('person').out('isLocatedIn').<String>values('name').as('placeName').select('person', 'placeName').forEachRemaining({map -> placeNameMap.put((Vertex) map.get('person'), (String) map.get('placeName'))});

    universityInfoMap = [:];
    g.V().has(person_label, 'iid', within(matchListIds)).as('person').outE('studyAt').as('classYear').inV().as('universityName').out('isLocatedIn').as('cityName').select('person', 'universityName', 'classYear', 'cityName').by().by('name').by('classYear').by('name').forEachRemaining({map -> v = map.get('person');
    ...
    companyInfoMap =[:];
    g.V().has(person_label, 'iid', within(matchListIds)).as('person').outE('workAt').as('workFrom').inV().as('companyName').out('isLocatedIn').as('cityName').select('person', 'companyName', 'workFrom', 'cityName').by().by('name').by('workFrom').by('name').forEachRemaining({map -> v = map.get('person')});
    ...
    if (companyInfoMap.containsKey(v)) {
      ...
      companyInfoMap.put(v, tupleList)
            ...
            ['matchList':matchList, 'distList':distList, 'propertiesMap':propertiesMap, 'placeNameMap':
    placeNameMap, 'universityInfoMap':universityInfoMap, 'companyInfoMap':companyInfoMap];
